...
class DatabaseCalendarFiller:
    """
    Jak pisałem mój pierwszy program, to Michał (@MIchalPiotrK)
    z TT mi wtedy pomógł z poprawniem/napisaniem kodu,
    ...
    def addRecord(self, table_name, year, month, day):
        ...
        self.__cur.execute(insert_command,
                           (stipend_type[0],
    ...
    def getData(self):
        self.__open_connection()
        select_command = "SELECT * FROM main_table"
        try:
            result = self.__con.execute(select_command)
...
class Month:
    ...
    def addRecords(self, table_name, qdatabase, days1to6, days7):
        """
        ...
        for day in range(1, days_in_month[-1] + 1):
            for recordIdx in range(days7 if self.isSunday(day) else days1to6):
                qdatabase.addRecord(table_name, self.year, self.month, day)
